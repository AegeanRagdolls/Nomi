...
| 分类 | Tabler 图标 import |
|---|---|
| 分镜 | `IconLayoutGrid` 或 `IconClapperboard` |
| 角色 | `IconUsers` |
| 场景 | `IconLandscape` 或 `IconMountain` |
| 道具 | `IconBox` |
| 声音 | `IconVolume` 或 `IconMusic` |
...
- 圆角：`var(--nomi-radius)`
- 阴影：`var(--nomi-shadow-sm)`，选中时 `var(--nomi-shadow-md)` + 描边 `var(--nomi-accent)`
- 默认尺寸：`width 320px × height 180px`（分镜）/ `260 × 160`（角色/场景/道具卡）/ `400 × 80`（声音条）

**分镜节点（ShotFrameNode）**：
```
┌─────────────────────────────────┐
│ 分镜 01           ⋯ 📋          │ ← 头部：编号 + 菜单 + 副本/派生角标
├─────────────────────────────────┤
│                                 │
│       [图像 / 占位灰底]          │ ← 主体：64% 高度
│       "分镜 01"                 │     占位态显示编号 + "等待生成"
...
│                                 │
├─────────────────────────────────┤
│ [prompt 单行 textarea]  [▶生成] │ ← 内嵌 composer (永久显示)
└─────────────────────────────────┘
```
...
│ [缩略图 / 占位]   ⋯ 📋          │
│                                 │
│ 小苏              [tag] [tag]   │ ← 名字 + 简短 tag
│ 反派少年，14 岁，有伤疤          │ ← 一句话设定（可空）
└─────────────────────────────────┘
```
...
- 灰底色：`var(--nomi-paper-2)` 或 `#F2F0EC`
- 占位文字（居中）：
  - 第一行：分类名 + 编号，如 "分镜 01" / "角色 03"
  - 第二行（小字，灰）："等待生成"
- **删除当前棋盘背景**（如有）
...
```typescript
import { IconClapperboard, IconUsers, IconLandscape, IconBox, IconVolume } from '@tabler/icons-react'
